...
export class PreconditionContainerArray implements IPreconditionContainer {
    ...
    public messageRun(data: proto.IWebMessageInfo, args: ArgumentStream, command: Command, context?: PreconditionContext | undefined): PreconditionContainerReturn {
        return this.mode === PreconditionRunMode.Sequential
            ? this.condition.messageSequential(data, args, command, this.entries, context)
            : this.condition.messageParallel(data, args, command, this.entries, context);
    }
