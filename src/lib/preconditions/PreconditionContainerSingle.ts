...
export class PreconditionContainerSingle implements IPreconditionContainer {
    ...
    public messageRun(data: proto.IWebMessageInfo, args: ArgumentStream, command: Command, context?: PreconditionContext | undefined): Awaitable<Result<unknown, UserError>> {
        const precondition = container.stores.get("preconditions").get(this.name);
        if (precondition) {
            return precondition.messageRun
                ? precondition.messageRun(data, args, command, { ...context, ...this.context })
                : precondition.error({
