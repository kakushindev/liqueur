...
export class PreconditionStore extends Store<Precondition> {
    ...
    public async messageRun(data: proto.IWebMessageInfo, args: ArgumentStream, command: Command, context: PreconditionContext = {}): Promise<Result<unknown, UserError>> {
        for (const precondition of this.globalPreconditions) {
            const result = precondition.messageRun
                ? await precondition.messageRun(data, args, command, context)
